...
LeapServer::~LeapServer() { controller.removeListener(*this); }
...
void LeapServer::sendData(const Direction &direction) {
  std::string directionToString[] = {"FORWARD", "BACKWARD", "RIGHT", "LEFT",
                                     "UNDEFINED"};

  assert(EnvoieMessage(this->getClient()[0], (char *)"%s\n",
